...
final class HeadSetupPanelTests: XCTestCase {
    @MainActor func testHostingControllerCannotCollapseSetupWindowToZeroSize() throws {
        ...
        defer { panel.close() }
        let content = NSHostingController(rootView: Text("Head tracking").frame(width: 440, height: 568))
        content.sizingOptions = []
        panel.installContent(content)
        panel.contentView?.layoutSubtreeIfNeeded()
        let bounds = try XCTUnwrap(panel.contentView?.bounds)
        XCTAssertEqual(bounds.width, 440, accuracy: 1)
        XCTAssertEqual(bounds.height, 568, accuracy: 1)
        XCTAssertGreaterThan(panel.frame.height, 568)
